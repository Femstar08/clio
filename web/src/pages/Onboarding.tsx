...
export default function Onboarding() {
  ...
  const { config, updateConfig, updateTokens } = useConfig();
  ...
  const handleComplete = async () => {
    ...
    // Save API key to .env via tokens endpoint
    if (apiKey) {
      const tokenKey = provider === "openai" ? "OPENAI_API_KEY" : "OPENROUTER_API_KEY";
      await updateTokens({ [tokenKey]: apiKey });
    }
